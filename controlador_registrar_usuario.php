...
if (isset($_POST['registro'])) {
    ...
    // Generar código de verificación de 6 dígitos aumente esto
    $codigo_verificacion = sprintf("%06d", mt_rand(1, 999999));
    
    // Encriptar la contraseña
    $contraseña_hash = password_hash($contraseña, PASSWORD_DEFAULT);
            // Insertar en la base de datos
    $query = "INSERT INTO usuario (Nombre, Apellido, Correo, Contraseña, Telefono, Genero, Id_pais, Id_ciudad, Rol, token_verificacion, verificado, codigo_verificacion) 
              VALUES ('$nombre', '$apellido', '$correo', '$contraseña_hash', '$telefono', '$genero', '$id_pais', '$id_ciudad', '$rol', '$token', 0, '$codigo_verificacion')";

    if (mysqli_query($conexion, $query)) {
        // Enviar correo de verificación
        $resultado_envio = enviarCorreoVerificacion($correo, $nombre . ' ' . $apellido, $token, $codigo_verificacion);
        ...
        if ($resultado_envio === true) {
           // $_SESSION['mensaje'] = array('tipo' => 'success', 'texto' => 'Registro exitoso. Por favor, verifica tu correo electrónico para activar tu cuenta.');
            header('Location: validar.php?estado=esperando&email=' . urlencode($correo));
            exit;   
        } else {
